...
module.exports.setKeyValueInConfig = async function (key, value) {
  return new Promise((resolve, reject) => {
    let installationPath = process.env.INSTALL_PATH;
    fs.readFile(installationPath + '/config.yaml', 'utf8', function (
      err,
      data
    ) {
      if (err) {
      ...
      fs.writeFile(installationPath + '/config.yaml', result, 'utf8', function (
        err
      ) {
        if (err) {
          reject(err);
...
module.exports.cleanST = async function () {
  ...
  await module.exports.executeCommand(
    'cd ' + installationPath + ' && rm -rf .webserver-temp-*'
  );
  await module.exports.executeCommand(
    'cd ' + installationPath + ' && rm -rf .started'
  );
};
...
async function getListOfPids() {
  ...
  try {
    (
      await module.exports.executeCommand(
        'cd ' + installationPath + ' && ls .started/'
      )
    ).stdout;
  } catch (err) {
    return [];
  ...
  for (let i = 0; i < currList.length; i++) {
    ...
    try {
      let pid = (
        await module.exports.executeCommand(
          'cd ' + installationPath + ' && cat .started/' + item
        )
      ).stdout;
      result.push(pid);
    } catch (err) {}
